...
import { PassionCategory, Passion, PassionCategoryEdit } from '../../../../../ducks/passions/types';
...
const EditablePassions: FC<PassionCategoryEdit> = ({ passions, id, isShowVisible, toggleOther, onPreesShow }) => {
  ...
  return (
    <>
      ...
      <TouchableOpacity style={{ flexDirection: 'row' }} onPress={toggleOther}>
        <Text
          style={[
            ...
            {!loading ? (
              data.map((item) => (
                <TouchableOpacity
                  ...
                  key={'id' in item ? item.id : item.name}
                  onPress={() => {
                    const finlData = updateData.filter((element) => {
